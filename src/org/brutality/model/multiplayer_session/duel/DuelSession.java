package org.brutality.model.multiplayer_session.duel;
...
public class DuelSession extends MultiplayerSession {
		...
		switch (stageId) {
			case MultiplayerSessionStage.OFFER_ITEMS:
				...
				for (Player p : players) {
	    			...
	    			if (overlap != null) {
	    				p.getPA().sendString("Too many of one item! The other player has "+Misc.getValueRepresentation(overlap.getAmount())+" "+player.getItems().getItemName(overlap.getId())+" in their inventory.", 6684);
			...
			case MultiplayerSessionStage.CONFIRM_DECISION:
				if (stage.hasAttachment() && stage.getAttachment() != player) {
					...
					player.getPA().createPlayerHints(10, opponent.index);
					opponent.getPA().createPlayerHints(10, player.index);
	...
	public void updateOfferComponents() {
		for (Player player : items.keySet()) {
    		...
    		player.getPA().sendString("Dueling with: " + recipient.playerName + " (level-" + recipient.combatLevel
					+ ")", 6671);
		}
	}
			...
			for (Player player : players) {
				...
				for (GameItem item : items) {
					if (item.getId() > 0 && item.getAmount() > 0) {
						itemList.append(player.getItems().getItemName(item.getId())
							+ " x "+Misc.getValueRepresentation(item.getAmount()) + "\\n");
					}
				}
		...
		if (!Objects.equals(getOther(winner.get()), winner.get())) {
			...
			if (items.get(winner.get()).size() > 0) {
				for (GameItem item : items.get(winner.get())) {
					...
					if (winner.get().getItems().freeSlots() == 0 || winner.get().getItems().playerHasItem(item.getId())
	...
	public void toggleRule(Player player, Rule rule) {
		...
		if (rule.equals(Rule.NO_MELEE) || rule.equals(Rule.NO_RANGE) || rule.equals(Rule.NO_MAGE)) {
			long count = rules.rules.stream().filter(r -> r.equals(Rule.NO_MELEE) || r.equals(Rule.NO_RANGE)
	...
	private void clearPlayerAttributes(Player player) {
		...
		player.hasBeenPoisoned = false;
		player.hasBeenVenomed = false;
		player.lastCast = 0;
        player.vengOn = false;
	}
	...
	/*private String createQuery(ArrayList<Entry<Player, String>> participantList, MultiplayerSessionFinalizeType type) {
		...
		String query = "INSERT into stakes (DATE, STAKESTATUS, WINNER, PLAYER, IP, STAKED, OTHERPLAYER, OTHERIP, OTHERSTAKED) VALUES(NOW(), '"
				+ status + "', '" + winner.get().playerName + "', '" + player.playerName + "', '" + getIp(player) + "', '" + items + "', '"
				+ playerOther.playerName + "', '" + getIp(playerOther) + "', '" + itemsOther + "')";
		return query;
	}*/
