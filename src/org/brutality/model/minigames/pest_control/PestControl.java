...
public class PestControl {
	...
	private static final int[][] NPC_DATA = {
		{ 1747, 2628, 2591, 250, 0, 150 }, // portal
			{ 1747, 2680, 2588, 250, 0, 150 }, // portal
			{ 1746, 2669, 2570, 250, 0, 150 }, // portal
			{ 1746, 2645, 2569, 250, 0, 150 }, // portal
	};
	...
	public static void drawInterface(Player player, String type) {
		...
		case "game":
			...
			for (int i = 0; i < NPC_DATA.length; i++) {
				...
				String color = npc.isDead ? "@red@" : npc.HP > 0 && npc.HP < 150 ? "@or1@" : "@gre@";
				int hp = npc.isDead ? 0 : npc.HP;
	...
	public static void killAllNpcs() {
		for (int i = 0; i < NPC_DATA.length; i++) {
			...
			if (npc != null) {
				NPCHandler.npcs[npc.index].applyDead = true;
				NPCHandler.npcs[npc.index].updateRequired = true;
				NPCHandler.npcs[npc.index] = null;
			}
		}
	}
