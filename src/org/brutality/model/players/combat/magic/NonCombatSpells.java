	...
	public static void playerAlching(Player c, int spell, int itemId, int slot) {
		switch (spell) {
			...
			if(System.currentTimeMillis() - c.alchDelay > 1000) {
				...
				int reward = ItemDefinition.forId(itemId).getLowAlchValue();
				...
				if(c.getItems().playerHasItem(itemId, 1, slot)){
				c.sendMessage("" + ItemDefinition.forId(itemId).getLowAlchValue());
				c.getItems().deleteItem(itemId, slot, 1);
				c.getItems().addItem(995, ItemDefinition.forId(itemId).getLowAlchValue());
			...
			if(System.currentTimeMillis() - c.alchDelay > 2000) {	
				...
				int reward = ItemDefinition.forId(itemId).getHighAlchValue();
				int playerAmount = c.getItems().getItemAmount(995);
				if(reward + playerAmount > 2147483647) {
				...
				if(c.getItems().playerHasItem(itemId, 1, slot)){
				c.sendMessage("" + ItemDefinition.forId(itemId).getHighAlchValue());
				c.getItems().deleteItem(itemId, slot, 1);
				c.getItems().addItem(995, ItemDefinition.forId(itemId).getLowAlchValue());
