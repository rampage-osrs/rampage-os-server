...
public class RandomEventInterface extends CycleEvent {
	...
	/**
	 * Creates a new event by randomly selecting a combination of items and choosing
	 * the item the player must select. The time is reset and the state of the event
	 * is set to active.
	 */
	public void execute() {
	...
	public void clickButton(int buttonId) {
		...
		if (slot != -1) {
			...
			if (item.getId() != correctItem.getId()) {
				...
				player.sendMessage("Incorrect, you have been sent to the jail.");
	...
	private enum Items {
		...
		/**
		 * An array of items that will be sent to the interface. One item the player
		 * must select.
		 */
