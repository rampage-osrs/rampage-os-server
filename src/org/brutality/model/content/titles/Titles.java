...
/**
 * The purpose of this class is to maintain any and all titles a single player may have
 * the rights to. 
 * 
...
public class Titles {
	...
	/**
	 * A single instance of the {@linkplain TitleRequirement} interface that does not 
	 * implement the <b>meetsRequirement(Client)</b> function.
	 */
	...
	/**
	 * This function will sort all items in the menu in an order that is relative to the
	 * titles purchability or current ownership.
	 */
	public void display() {
		List<Title> bought = populateMenu(
				0,
				(byte) 2, 
				t -> isPurchased(t) && t.getRequirement().meetsStandard(player)
		);
		List<Title> undisplayable = populateMenu(
				bought.size(), 
				(byte) 3, 
				t -> isPurchased(t) && !t.getRequirement().meetsStandard(player)
		);
		populateMenu(
				bought.size() + undisplayable.size(),
				(byte) 1,
				t -> !isPurchased(t)
		);
		view(Title.NONE);
	...
	public boolean click(int buttonId) {
		/*if(player.getRights().getValue() <= 1) {
			player.sendMessage("This is a donator only feature! ::donate");
			return false;
		}*/
		if (buttonId == 114091) {
			display();
	...
	private void select(Title title) {
		...
		if (title.equals(Title.CUSTOM)) {
			...
			if (temporaryCustomTitle.equalsIgnoreCase("admin") || temporaryCustomTitle.equalsIgnoreCase("mod")
					|| temporaryCustomTitle.equalsIgnoreCase("staff")) {
				player.sendMessage("You cannot use this custom title because it represents a staff title.");
				return;
			}
			Optional<Title> illegalMatch = Title.TITLES.stream().filter(t -> t.getName().equalsIgnoreCase(
					temporaryCustomTitle)).findFirst();
		...
		currentTitleColor = title.getColor();
		player.sendMessage("Your title has been changed to '"+title.name()+"'.");
		player.getPA().requestUpdates();
	}
	
	/**
	 * Determines if we have already purchased the title by determining if any of the 
	 * titles we currently have match the name of that title.
