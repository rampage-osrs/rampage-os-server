...
public class PotionMixing {
	...
	Player c;
	private static int[][] potMixing = new int[][] { { 117, 117, 113, 227 },
	...
	public static void mixPotion2(Player c, int id, int id2) {
		if (id >= 0 && id <= 30000 && id2 >= 0 && id2 <= 30000) {
			boolean def1 = ItemDefinition.forId(id).isNoted();
			boolean def2 = ItemDefinition.forId(id2).isNoted();
			if (!def1 && !def2) {
				...
				if (id11.contains("(") && id22.contains("(")) {
					if (id11.substring(0, id11.indexOf("(")).equalsIgnoreCase(
						...
						try {
							int e = Integer.parseInt(id11.substring(
									id11.indexOf("(") + 1,
									id11.indexOf("(") + 2));
							...
							if (totalAmount > 4) {
								...
								item1 = id11
										.substring(0, id11.indexOf("(") + 1)
										+ e1 + ")";
								String item2 = id11.substring(0,
										id11.indexOf("(") + 1)
										+ amount2 + ")";
	...
	public static void mixPotion(Player c, int id, int id2) {
		for (int j = 0; j < potMixing.length; ++j) {
			if (!ItemDefinition.forId(id).isNoted() && !ItemDefinition.forId(id2).isNoted()
					&& potMixing[j][0] == id && potMixing[j][1] == id
