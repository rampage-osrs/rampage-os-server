package org.brutality.net.outgoing;
...
/**
 * Some outgoing packets don't need to be sent to the client because at time,
 * information being sent hasn't changed. For example, in packet 126 we send a string
 * of characters and a component id. The string replaces the current string on the 
 * interface component. At times we send the same string to the client which is unncessary.
 * 
 * When a packet is to be sent, we will determine under some circumstances if it should
 * be dropped, or it if should be sent. We do this by matching up the credentials of the
 * old existing information last sent, and the message we are sending now. 
 * 
 * @author Jason MacKeigan
 * @date Nov 15, 2014, 3:40:52 AM
 */
