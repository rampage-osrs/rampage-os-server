...
public class Smithing {
	...
	private static void CheckAddy(final Player c, final int level,
	...
	private static void CheckBronze(final Player c, final int level,
			final int amounttomake, final String type) {
	...
	private static void CheckIron(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2351;
	...
	private static void CheckMith(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2359;
	...
	private static void CheckRune(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2363;
	...
	private static void CheckSteel(final Player c, final int level,
			final int amounttomake, final String type) {
		remove = 2353;
	...
	public static void doAction(final Player c, final int toadd, final int toremove,
		...
		c.animation(898);
		if (c.playerSkilling[Player.playerSmithing] == true) {
			return;
		}
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				if (c.playerSkilling[Player.playerSmithing] == false) {
					container.stop();
					return;
				}
	...
	public static void readInput(final int level, final String type, final Player c,
	...
	public void smelt(final int barType) {
		if (c.smeltAmount > 0) {
			...
			if (hasOres(barType)) {
				...
				c.getPA().addSkillXP(exp * Config.SMITHING_EXPERIENCE,Player.playerSmithing);
