...
public abstract class ItemCombination {
	 ...
	 */
	public ItemCombination(Optional<int[]> levelRequirements, GameItem outcome, Optional<List<GameItem>> revertedItems, GameItem...items) {
	...
	public void revert(Player player) {
		...
		if (player.getItems().freeSlots() < revertedItems.get().size()) {
			player.getDH().sendStatement("You need atleast "+revertedItems.get().size()+""
		...
		player.getDH().sendStatement("The "+player.getItems().getItemName(outcome.getId())+" has been split up.",
	...
	public void insufficientRequirements(Player player) {
		...
		player.getDH().sendStatement("You must have a " + Skill.forId(getSkillRequirement()) +
	...
	public boolean isCombinable(Player player) {
		Optional<GameItem> unavailableItem = items.stream().filter(i -> !player.getItems().playerHasItem(i.getId(),
	 ...
	 */
	public boolean allItemsMatch(GameItem item1, GameItem item2) {
	...
	public boolean isRequirable() { 
		return levelRequirements.isPresent();
	}
